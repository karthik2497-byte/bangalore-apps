...
### 4.2 LingoLocal (ship first — zero backend)
- P0: Pre-recorded audio for the ~90 phrases (record once, ~2MB of .opus files, bundle in SW cache) — TTS Kannada voices are often missing on devices; audio files are the quality moat.
- P1: Spaced-repetition ordering for flashcards (simple SM-2-lite on localStorage timestamps).
- P1: 3 more scenario packs (office, hospital, temple/festival etiquette) — content is the product.
