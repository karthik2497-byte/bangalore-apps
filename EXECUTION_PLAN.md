...
| 2026-09-07 | Claude (Opus 5) | all 7 (deploy config), StockPing (1-line fix) | Origin decided and wired: `tinkerhouse.uk` bought, one origin with a subpath per app. Added `/.well-known/assetlinks.json` (7 statements, one per TWA) and made `build-site.sh` ship it — Digital Asset Links must sit at the origin root, which is how 7 Play listings verify against 7 paths on one host. `build-aab.yml` no longer hardcodes the github.io origin: new `origin` input, plus a pre-build `curl` guard that fails if the origin does not serve that app's manifest and name its package in assetlinks. **Caught a collision:** bubblewrap derives `packageId` from the host, so all 7 apps on one origin would have generated the same id and only one could ever exist on Play — `generate-twa.mjs` now takes an explicit package id and each app passes `uk.tinkerhouse.<app>`. StockPing SW `openWindow('/')` → `'./'` (absolute path would leave the app's subpath); checked every caller, CardGuard's was already relative. | 5/5 test suites PASS; `node --check` on the changed SW + `generate-twa.mjs` PASS; `build-site.sh` OK (153 files, all 7 package ids present in `dist/.well-known/assetlinks.json`). **Then deployed and verified live:** Pages project `tinkerhouse` created, `tinkerhouse.uk` bought and attached, `bash scripts/verify-deploy.sh https://tinkerhouse.uk` → PASS (7 apps, per-app CSPs applied with the font origins in font-src AND connect-src, X-Frame-Options, valid TLS, http→https 301, assetlinks 7/7 at the origin root, must-revalidate on both live JSON configs, real 404s, no repo file reachable). Script proven to fail against an unrelated origin, so the PASS means something. | Two found in my own work, both fixed: `build-site.sh` used `rsync`, which the Pages build image does not have (first deploy died, exit 127); and the site shipped no `404.html`, so Pages soft-404'd every unmatched path with the root page at HTTP 200. |
| 2026-09-07 | Claude (Opus 5) | none (docs only) | Session close. Added `NEXT_STEPS.md` — the standing "what is still open" index — and imported it from `CLAUDE.md` alongside `@MISTAKES.md` so every future session loads it without a read step. Corrected Phase A in this file from "🟡 repo side done, account side needs Karthik" to ✅ DONE: the Pages project, the `tinkerhouse.uk` domain and assetlinks all landed in commits `762bbd7`…`35d827d` after that status was written. No app code touched. | `bash scripts/verify-deploy.sh` → **PASS** against the live origin: 7 apps served, per-app CSPs applied with the font origins in both `font-src` and `connect-src`, `X-Frame-Options: DENY`, assetlinks 7/7, real 404s, no repo file reachable. `git status --short` before writing: only my own two files dirty. | None. Stale status corrected, not introduced: Phase A item 1 still read ❌ because the live-origin note was prepended as item 0 rather than fixing the item it superseded. Also confirmed the 8 open `impeccable` design findings all trace to `446e6c1` (2026-04-09) / `012801f` (2026-04-10) — pre-existing, recorded in `NEXT_STEPS.md` §6, and **no ignore command has been run**, since suppression needs Karthik's explicit confirmation. |
...
### Phase A — Web live (Week 1) — ✅ DONE 2026-09-07, live at `https://tinkerhouse.uk`
...
1. ✅ Cloudflare Pages project created and repo-connected (Karthik did the login and the OAuth grant; an agent must not). Settings in use:
...
2. ✅ Domain bought: **`tinkerhouse.uk`** (Cloudflare Registrar, $5.30/yr flat). Deliberately not geography- or category-named, so non-Bangalore and non-app work can live here later. One origin, subpath per app (`tinkerhouse.uk/2_meter_smart/`) — not subdomains, because `_headers` is path-scoped and cross-origin would split each app's localStorage. Android package ids are `uk.tinkerhouse.<app>`, fixed permanently at first Play publish.
